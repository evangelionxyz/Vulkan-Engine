...
VulkanBuffer::VulkanBuffer(const void *data, VkDeviceSize size, VkBufferUsageFlagBits usage)
{
    ...
    result = vkAllocateMemory(device, &alloc_info, VK_NULL_HANDLE, &m_Memory);
    VK_ERROR_CHECK(result, "[Vulkan] Failed to allocate vertex buffer memory");

    set_data(data, size);
}

void VulkanBuffer::set_data(const void *data, VkDeviceSize size)
{
    const VkDevice device = VulkanContext::get()->get_device();
    vkBindBufferMemory(device, m_Buffer, m_Memory, 0);
    copy_data_to_buffer(device, m_Memory, data, size);
}
